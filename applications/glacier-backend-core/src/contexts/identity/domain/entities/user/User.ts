 ...
 *
 * The User aggregate emits domain events for significant state changes:
 * - {@link UserRegisteredEvent} when a new user is registered
 * - {@link UserSuspendedEvent} when the user is suspended
 * - {@link UserActivatedEvent} when the user is activated
 *
 * Collect uncommitted events with {@link User#collectEvents} for event sourcing
 * or @nestjs/cqrs integration.
 */
export class User {
  ...
  /** Uncommitted domain events emitted by this aggregate. */
   ...
   * Emits {@link UserSuspendedEvent} after successful state change.
   *
  ...
  public suspend(at: Date): void {
    ...
    this.touch(at);
    this.domainEvents.push(new UserSuspendedEvent(this.id, at));
  }
   ...
   * Emits {@link UserActivatedEvent} after successful state change.
   *
  ...
  public activate(at: Date): void {
    ...
    this.touch(at);
    this.domainEvents.push(new UserActivatedEvent(this.id, at));
  }
  ...
  /**
   * Collects and clears all uncommitted domain events from this aggregate.
   *
   * Useful for event sourcing or @nestjs/cqrs integration. After collecting events,
   * they are removed from the aggregate's internal collection to prevent double publishing.
   *
   * @returns An array of {@link DomainEvent} instances emitted by this aggregate.
   */
  public collectEvents(): DomainEvent[] {

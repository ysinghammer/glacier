...
> {
   ...
   * @throws {InvalidUserDataException} If email or name validation fails.
   * @throws {UserAlreadyExistsException} If the new email already exists for a different user.
   */
  public async execute(command: UpdateAuthUserCommand): Promise<UpdateAuthUserCommandResult> {
    ...
    if (command.firstName !== undefined && command.lastName !== undefined) {
      try {
        ...
        throw new InvalidUserDataException('name', message);
      }
    }
    ...
    if (command.status !== undefined) {
      if (command.status === 'active') {
        ...
        throw new InvalidUserDataException(
          'status',
          `Must be "active" or "suspended", got "${command.status}"`
        );
      }
    }
